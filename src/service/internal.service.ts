...
export const internalService = {
  ...
  updateTagByManyHeroes: async (heroIds: string[], tagIds: string[]) => {
    const heroes = await Hero.find({ _id: { $in: heroIds } })
  ...
  deleteTagByManyHeroes: async (heroIds: string[], tagIdsToRemove: string[]) => {
    const heroes = await Hero.find({ _id: { $in: heroIds } })

    const updatedHeroes = await Promise.all(
      heroes.map(async (hero) => {
        const filteredTags = hero.tags.filter((tagId: any) => {
          return !tagIdsToRemove.includes(tagId)
        })
        hero.tags = filteredTags
        await hero.save()
        return hero
      })
    )
